#!/usr/bin/env python3
"""
Modified RAG application example that uses PDFFileLoader instead of TextFileLoader.
This demonstrates how to read PDF files instead of text files in the RAG pipeline.
"""
...
# Import the modified text_utils with PDFFileLoader
...
def load_pdf_documents(pdf_path: str, use_pdfplumber: bool = True):
    """
    ...
    try:
        pdf_loader = PDFFileLoader(pdf_path, use_pdfplumber=use_pdfplumber)
...
"""
Default Values:
response_style: Defaults to "detailed" in the constructor
response_length: Defaults to "detailed" in the run_pipeline method if not provided
Where to Modify:
If you want to change the default values or add more options, you can modify:
Constructor default: Change response_style: str = "detailed" in the __init__ method
Runtime default: Change system_kwargs.get("response_length", "detailed") in the run_pipeline method
Template text: Modify the instruction text in RAG_SYSTEM_TEMPLATE to be more specific about what these parameters mean
The system will use these values to format the prompt that gets sent to the LLM, helping it understand how to structure and style its response.
"""
...
def main():
    ...
    # Replace this path with your actual PDF file
    ...
    vector_db = asyncio.run(build_vector_database(split_docs))
